...
export const UserWithdrawal = () => {
  ...
  useEffect(() => {
    getWithdrawal(`user/withdrawRequest?userId=${localStorage.getItem("userid")}`, setwithdrawal)
  },[])
  ...
  return (
    <div className=" px-8 py-5">
      ...
      {withdrawal ? (withdrawal.map((withdraw, index)=> (
      <div className="w-full h-auto shadow-sm grid grid-cols-5 grid-rows-1 text-center rounded py-3 border-b-2 text-[14px] bg-white">
        ...
        <p className=" flex justify-center items-center ">
          <p
            className=" bg-yellow-300 px-3.5 rounded-full border-2 border-yellow-600 py-1"
          >
            {withdraw.status}
          </p>
        </p>
