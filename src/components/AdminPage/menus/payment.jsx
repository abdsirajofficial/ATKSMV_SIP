...
const accepReq = (id) => {
  ...
  return (
    <div className=" px-8 py-5">
      <div
        className="w-full h-auto shadow-sm grid grid-cols-6 grid-rows-1 text-center rounded py-3  text-[14px] bg-white mb-5"
      >
      ...
      <div className="w-full h-auto shadow-sm grid grid-cols-6 grid-rows-1 text-center rounded py-3  text-[14px] bg-white border-b-2">
        ...
        <p className=" flex justify-center items-center">{data.updatedOn.slice(0, 10)}</p>
        <p className=" flex justify-center items-center ">
          <p
            className={`px-3.5 rounded-full border-2 py-0.5 ${
              data.status === "Success" ? "bg-green-50 border-green-200" : "bg-yellow-50 border-yellow-200"
            }`}
          >
            {data.status}
          </p>
        ...
        {data.status === "Pending" && (
          <p className=" flex justify-center items-center space-x-5">
              <button className=' flex  justify-center items-center space-x-2 bg-red-600 px-2 py-1.5 rounded-lg text-white hover:scale-110'
            //   onClick={() => deleteUser(user.userId)}
              >
                <p  className=" text-[20px]" ><MdDelete /></p>
                <p>Delete</p>
              </button>
              <button
              ...
              >
                <p  className=" text-[20px]" ><VscWorkspaceTrusted /></p>
                <p>Accept</p>
              </button>
            </p>
        )}
